...
- **目标 3**: 实现**动态 System Prompt 组装机制**——从用户可编辑的 `RMAN.md` 和 `TOOLS.md` 文件中读取内容，运行时动态构建最终的 System Prompt，使 r-man 的行为可以在不修改代码的情况下灵活定制。
...
**描述**: 为了防止 Agent 产生非预期的破坏性行为，系统必须实施多层安全防护。

#### 3.8.1 目录隔离与路径保护
- **Workspace 锁定**: 内部工具 `read`, `write`, `replace` 以及 `run_shell_command` 的 `dir_path` 参数必须被严格限制在 `workspace/` 目录下。任何尝试访问父目录（`..`）或系统绝对路径的操作必须被代码层拦截并返回 `Permission Denied`。

#### 3.8.2 核心配置文件保护
- **RMAN.md & TOOLS.md 锁定**: 禁止 `write_file` 和 `replace` 工具对工作区下的 `RMAN.md` 和 `TOOLS.md` 进行任何写操作。这两个文件被视为 Agent 的“DNA”，仅允许通过用户手动编辑来修改。

#### 3.8.3 破坏性操作人工确认
- **高危操作定义**: 包括 `rm`, `kill`, `truncate`, `mv`（跨目录移动）等可能导致数据丢失或服务中断的操作。
- **确认流程**:
    1.  当任务涉及高危操作时，Agent 必须停止当前工具调用。
    2.  在 `<final>` 标签中向用户解释操作计划及潜在风险。
    3.  显式等待用户回复“确认”。
    4.  只有检测到用户回复中包含“确认”字样时，方可在下一轮迭代中执行该操作。
