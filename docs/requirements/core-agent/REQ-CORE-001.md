...
#### 3.1.5 观察值溢出保护 (Observation Overflow Protection)
- **硬截断阈值**: 单次工具调用产生的 `Observation` 文本如果超过 40,000 字符（约 15K-20K Tokens），严禁全量注入上下文。
- **智能蒸馏**: 超限时必须执行“三段式蒸馏”：
    1. **Head**: 保留前 1,000 字符。
    2. **Summary**: 调用专门的极简 Prompt 生成中间万级数据的特征摘要（含错误模式、数据规模）。
    3. **Tail**: 保留最后 2,000 字符。
- **Agent 引导**: 在 Observation 结尾附加警告，引导 Agent 使用分页或重定向工具处理大数据。
