...
| v1.21.0 | 2026-04-29 | download_image 工具，支持将飞书资源下载至本地服务器 | Gemini CLI |
| v1.22.0 | 2026-05-15 | 引入 tiktoken 进行精确 Token 统计；增强解析鲁棒性；落地摘要详略程度动态调整机制 | Gemini CLI |
...
- **核心参数**:
    ...
    - **统计精度**: **必须使用 `tiktoken` 库**进行精确统计（或其针对特定模型的对等方案），严禁使用 `char // 2` 等粗略估算，以防 context 溢出或过早压缩。
    - **触发**: 当 Token 数 > `context_window * 0.8` (约 102K) 时。
    - **压缩逻辑**:
        1.  **锚点**: 锁定 `System Prompt` (Index 0) 和最近 5 轮消息 (Tail)。
        2.  **预算分配**: 根据目标 60% 阈值与锚点占用情况，动态计算摘要可用的 Token 预算。
        3.  **摘要化**: 调用 LLM 专门的“压缩指令”生成技术摘要。**详略程度要求**：系统必须根据可用预算动态调整摘要的详略程度（例如，预算充足时提供详细步骤，预算紧张时仅保留核心状态）。
        4.  **合并**: 将新生成的摘要追加到历史记录前端的 `[Compacted Summary]` 模块。
        5.  **目标**: 确保压缩后 Token < `context_window * 0.6` (约 76K)。


#### 3.1.4 解析鲁棒性 (Parsing Robustness)
- **标签解析**: 必须能够准确提取模型输出中的 `<think>`（思考过程）和 `<final>`（最终结论）标签。
- **容错处理**: 解析器必须支持标签的大小写不敏感（如 `<THINK>`），并能够容忍标签未闭合的情况（例如在输出被截断时，将开标签之后的所有内容视为该部分内容），确保 Agent 意图的最大程度提取。
